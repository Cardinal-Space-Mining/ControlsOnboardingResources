                                 ...
                                 const sensor_msgs::msg::Joy & ctrl)
{
    ...
    {
        if(ctrl.buttons[Ps4Mapping::Buttons::L_STICK]) 
        {
            this->set_state(State::Trench, state);
            return MotorSettings();
        }

        if(ctrl.buttons[Ps4Mapping::Buttons::R_STICK]) 
        {
            this->set_state(State::Offload, state);
            return MotorSettings();
        }

        if(ctrl.axes[Ps4Mapping::Axes::DPAD_U_D] ==
           Ps4Mapping::Axes::DPAD_K::DPAD_RIGHT)
        {
            this->set_state(State::Offload, state);
            return MotorSettings();
        }
        ...
        if(ctrl.axes[Ps4Mapping::Axes::DPAD_U_D] ==
           Ps4Mapping::Axes::DPAD_K::DPAD_UP)
        {
            this->set_state(State::Trench, state);
            return MotorSettings();
        }
    ...
    // Controls
    {
        ...
        // Hopper Up and Down
        {
            ...
            bot.hopper_actuator.value =
                -1.0 * apply_deadband(ctrl.axes[Ps4Mapping::Axes::RIGHTY],
        ...
        // Hopper Belt
        {
            double trigger_percentage =
                (ctrl.axes[Ps4Mapping::Axes::L_TRIGGER] + 1.0) / 2.0;
            if (ctrl.buttons[Ps4Mapping::Buttons::LB])
            {
        ...
        {
            double trigger_percentage =
                (ctrl.axes[Ps4Mapping::Axes::R_TRIGGER] + 1.0) / 2.0;
            if (ctrl.buttons[Ps4Mapping::Buttons::RB])
            {
                trigger_percentage *= -1.0;
            }
                                 ...
                                 const sensor_msgs::msg::Joy & ctrl)
{
    ...
    // Transitions
    {
        ...
        if(ctrl.buttons[Ps4Mapping::Axes::DPAD_U_D] ==
               Ps4Mapping::Axes::DPAD_K::DPAD_DOWN &&
           state.lifecycle != Lifecycle::End)
        {
            state.lifecycle = Lifecycle::End;
        }
    ...
    // Control
    {
        ...
        // Tracks
        {
            double tracks_speed_percent =
                (ctrl.axes[Ps4Mapping::Axes::LEFTY] + 1.0) / 2.0;
